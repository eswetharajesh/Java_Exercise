...
public class PaintJob {
    public static int getBucketCount(double width, double height, double areaPerbucket, int extraBuckets) {
        if ((width <= 0) || (height <= 0) || (areaPerbucket <= 0) || (extraBuckets < 0)) {
            ...
            double areaPerExtraBucket = areaPerbucket * extraBuckets;
            double remainingArea = area - areaPerExtraBucket;
            int remainingBucket = (int) Math.ceil((double) remainingArea / areaPerbucket);
    ...
    public static int getBucketCount(double area, double areaPerBucket) {
        if ((area <= 0) || (areaPerBucket <= 0)) {
            ...
            int TotBucket = (int) Math.ceil((double) area / areaPerBucket);
            return TotBucket;
        }
    }
